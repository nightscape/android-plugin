...
import AndroidHelpers.manifest
...
import AndroidPlugin.testRunner
...
object AndroidTest {
  ...
  private def testTask = (s: State) => loadForParser(definedTestNames in Test)((s, i) => testParser(s, i getOrElse Nil))
  /**
   * Test settings
   */
  lazy val settings: Seq[Setting[_]] =
    (Seq(
      ...
      testOnly <<= Def.inputTask(testTask)))
