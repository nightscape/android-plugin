...
object AndroidBase {
  ...
  // TODO can only handle tuples of size 11
  private def aaptGenerateTask =
    (manifestPackage, aaptPath, generatedProguardConfigPath, manifestPath,
      resPath, libraryJarPath, managedJavaPath, aarlibDependencies,
      apklibDependencies, apklibSourceManaged, streams) map { //, useDebug) map {

        (mPackage, aPath, proGen, mPath,
        rPath, jarPath, javaPath, aarlibs,
        apklibs, apklibJavaPath, s) => //, useDebug) =>
