...
import AndroidPlugin.hprofDevice
import AndroidPlugin.hprofEmulator
import AndroidPlugin.manifestPackage
import AndroidPlugin.manifestPackageName
import AndroidPlugin.screenshotDevice
import AndroidPlugin.screenshotEmulator
import AndroidPlugin.stopBridge
import AndroidPlugin.threadsDevice
import AndroidPlugin.threadsEmulator
...
object AndroidDdm {
      ...
      mask match {
        ...
        case Client.CHANGE_THREAD_DATA =>
          ...
          if (client.getClientData.getThreads != null) {
            val tnames = for (tinfo <- client.getClientData.getThreads) yield { tinfo.getThreadName }
            infos.retain((name, _) => tnames.contains(name))
          }
        case Client.CHANGE_THREAD_STACKTRACE =>
          if (client.getClientData.getThreads != null) {
            for (
              tinfo <- client.getClientData.getThreads; if tinfo.getStackTrace != null
            ) {
  ...
  def withClient[F](emulator: Boolean, path: String, clientPkg: String)(action: Client => F): Option[F] = {
  ...
  def dumpHprof(app: String, path: String, emulator: Boolean, streams: TaskStreams)(success: (Client, Array[Byte]) => Unit) {
  ...
  private def threadEmulatorTask = (s: State) => (resolvedScoped, dbPath) map {
    (ctx, path) => threadListParser(s, loadFromContext(manifestPackageName, ctx, s) getOrElse "", path.absolutePath, true)
  }

  private def threadsDeviceTask = (s: State) => (resolvedScoped, dbPath) map {
    (ctx, path) => threadListParser(s, loadFromContext(manifestPackageName, ctx, s) getOrElse "", path.absolutePath, false)
  }
  ...
  lazy val settings: Seq[Setting[_]] = (Seq(
    ...
    threadsEmulator <<= Def.inputTask(threadEmulatorTask),

    threadsDevice <<= Def.inputTask(threadsDeviceTask),
    //    threadsEmulator <<= InputTask(
    //      (resolvedScoped, dbPath)((ctx, path) => (s: State) =>
    //        threadListParser(s, loadFromContext(manifestPackageName, ctx, s) getOrElse "", path.absolutePath, true)))(printStackTask),
    //
    //    threadsDevice <<= InputTask(
    //      (resolvedScoped, dbPath)((ctx, path) => (s: State) =>
    //        threadListParser(s, loadFromContext(manifestPackageName, ctx, s) getOrElse "", path.absolutePath, false)))(printStackTask),
